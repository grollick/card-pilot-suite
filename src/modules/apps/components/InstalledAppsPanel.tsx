...
import InstalledAppSettingsDialog from "./InstalledAppSettingsDialog";
...
export default function InstalledAppsPanel() {
  ...
  const [settingsApp, setSettingsApp] = useState<InstalledApp | null>(null);
  ...
  return (
    <>
            ...
            <Card key={ia.id} className="border-border/60">
              <CardContent className="p-4 flex items-center gap-4">
                <div className="h-9 w-9 rounded-lg bg-muted flex items-center justify-center text-lg shrink-0">
                  {app.icon_url ? (
                    <img src={app.icon_url} alt={app.name} className="h-7 w-7 rounded-md object-contain" />
                  ) : "📦"}
                </div>
      ...
      <InstalledAppSettingsDialog
        installedApp={settingsApp}
