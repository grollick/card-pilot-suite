...
import { CheckCircle2, Rocket, ArrowRight, Lightbulb, PartyPopper, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useActivationChecklist } from "@/hooks/useActivationChecklist";
import { GROUP_LABELS } from "@/lib/checklistTemplates";
import { useEffect, useRef, useState } from "react";
import { toast } from "sonner";

export default function ActivationChecklist() {
  const navigate = useNavigate();
  const { template, items, completedCount, totalCount, progress, allDone, nextItem } =
  ...
  // Build milestone lookup from template
  const milestoneMap = new Map(template.milestones.map((m) => [m.key, m]));

  // Celebration effect
  useEffect(() => {
    if (items.length === 0) return;
    const currentDone = new Set(items.filter((c) => c.done).map((c) => c.key));
    const prev = prevCompletedRef.current;
    if (prev.size > 0) {
      for (const key of currentDone) {
        if (!prev.has(key) && milestoneMap.has(key)) {
          const msg = milestoneMap.get(key)!;
  ...
  return (
    ...
    >
      {/* Header — uses profession-specific headline */}
      <div className="flex items-center gap-3 mb-4">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xs font-bold text-primary tabular-nums">{completedCount}/{totalCount}</span>
          </div>
        </div>
        <div className="flex-1">
          ...
          <p className="text-2xs text-muted-foreground">{template.subheadline}</p>
        </div>
      </div>
