...
export default function SocialAnalyticsTab() {
  ...
  const platformCounts = PLATFORMS.map(platform => {
    const count = published.filter(p =>
      ((p.platforms_json as any) || []).includes(platform.id)
    ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Card className="dash-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Post Frequency by Day</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="rounded-lg border border-border p-3">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-xs text-muted-foreground ml-auto">
                    {format(new Date(topPost.created_at), "MMM d, yyyy")}
                  </span>
                </div>
              </div>
            </CardContent>
